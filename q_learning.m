...
for i = 1:NumIter
    ...
    while 1
        ...
        if epsilon <= exploration_factor    % Explore the grid! Do a random action.
            ...
            [maxq,action] = max(Q(taxi_position,:,passenger,destination)); % Play safe! Take the most optimal action.
            if(maxq == 0)
                ...
                if isempty(z)
                    ...
                    while 1
                        ...
                        if Q(taxi_position,action,passenger,destination) == 0
                            break;
                        end
                    end
                end
        ...
        current_reward = grid(taxi_position,action,passenger,destination); % Get the immediate reward value of the chosen action for the particular state
        if grid(taxi_position,action,passenger,destination) ~= -1   % The taxi doesn't bump into a wall.
            switch action
                ...
                case 6 % Action: Drop Passenger and calculate the final reward value.
                    ...
                    Q(taxi_position,action,passenger,destination) = Q(taxi_position,action,passenger,destination) + alpha * (grid(taxi_position,action,passenger,destination) + gamma*(max(Q(newpos,:,passenger,destination))) - Q(taxi_position,action,passenger,destination));
